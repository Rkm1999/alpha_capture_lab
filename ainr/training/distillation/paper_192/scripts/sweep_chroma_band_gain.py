#!/usr/bin/env python3
"""Sweep a gain applied only to the student's very-coarse chroma correction."""
...
from train_mixed import chroma_projection, gaussian_blur, validate  # noqa: E402


class ChromaBandGainModel(nn.Module):
    def __init__(
        self,
        model: nn.Module,
        gain: float,
        coarse_sigma: float,
        very_coarse_sigma: float,
    ) -> None:
        super().__init__()
        self.model = model
        self.gain = float(gain)
        self.coarse_sigma = float(coarse_sigma)
        self.very_coarse_sigma = float(very_coarse_sigma)

    def forward(self, model_input: torch.Tensor) -> torch.Tensor:
        noisy = model_input[:, :3]
        restored = self.model(model_input)
        correction_chroma = chroma_projection(restored.float() - noisy.float())
        band = gaussian_blur(
            correction_chroma,
            self.coarse_sigma,
        ) - gaussian_blur(correction_chroma, self.very_coarse_sigma)
        return (restored.float() + (self.gain - 1.0) * band).clamp(0.0, 1.0)
    ...
    for gain in args.gains:
        ...
        results[label] = validate(
            ChromaBandGainModel(
                base_model,
                gain,
                float(target_config["coarse_sigma"]),
